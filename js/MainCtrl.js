angular.module('MainCtrl', []).controller('MainController', function($scope, $http, $location) {
  ...
  var health = function() {
    $scope.label = "";
    $http.get('https://spreadsheets.google.com/feeds/list/1Iw8lzITqytYMshIs0kV31pHrxK6oFReo5zE9e3hQisI/1/public/values?alt=json').then(function success(res) {
      var rows = res.data.feed.entry;
      var $l = $scope.lines;
      var sep = " | ";
      $l.push("<i>Health Stats:</i>");
      lineBreak(); 
      $l.push("Date, Sleep, RHR")
      for (row in rows) {
        var current = rows[row]; 
        $l.push("<i>" + current.gsx$date.$t + "</i>" + sep + current.gsx$sleep.$t + sep + current.gsx$rhr.$t);
      }
      $scope.label = $scope.defaultLabel;
      lineBreak(); 
    }); 
  ...
  var about = function() {
    ...
    $scope.lines.push("* I am on the cross country team at Chico State");
    ...
    $scope.lines.push("* I use spaces, not tabs, and I don't care if you hate me for that");
    lineBreak();
    $scope.lines.push("My (dilapidated) <a target='_blank' href='https://www.instagram.com/rbreeze3/'>Instagram</a>"); 
  ...
  var workExperience = function() {
    ...
    $l.push("June 2017 - Present");
    $l.push("I am responsible for updating the hair salon's main website and migrating it to Wordpress. Recently my role has shifted to focus more on this migration, but in the past I have been responsible for creating content for social media and YouTube using Adobe Photoshop and Premiere Pro.");
    lineBreak(); 
    $l.push("<em>Web Developer and Communications Manager</em> at <i>Techlore</i>");
    $l.push("September 2017 - February 2018");
    $l.push("I created a website for the Techlore YouTube channel, and during this period I was also responsible for responding to comments and inquiries on the channel and all its videos.");
    lineBreak()
    $l.push("<em>Web Developer and Social Media Manager</em> at <i>First Choice Tutoring</i>");
    $l.push("November 2016 - April 2017");
    $l.push("I created a static website hosted on GitHub pages for First Choice Tutoring, a local tutoring company. I also managed the company's social media presence, posting daily to Instagram and Facebook and engaging with the community, which grew the company's following from about 100 followers to over 1000 in a period of around 3 months.");
    lineBreak();
    $l.push("<em>Associate</em> at <i>The Niche Ice Cream</i>");
    $l.push("August 2016 - June 2017");
    $l.push("Served Ice Cream and performed various cleaning duties. I started work when the business opened, and worked alongside the owner for many shifts. Therefore I learned a lot about customer engagement and marketing as I watched his business grow.");
    lineBreak(); 
    $l.push("<em>Cashier</em> at <i>Jimboy's Tacos</i>"); 
    $l.push("March 2016 - August 2016");
    lineBreak(); 
